...
def knowledge_base_page(api: ApiRequest, is_lite: bool = None):
    ...
    selected_kb = st.selectbox(
        "Please select or create a knowledge repository",
        kb_names + ["create a knowledge repository"],
        format_func=format_selected_kb,
        index=selected_kb_index
    )

    if selected_kb == "create a knowledge repository":
        with st.form("create a knowledge repository"):

            kb_name = st.text_input(
                "Name of new knowledge repository",
                placeholder="New knowledge repository name, Chinese naming not supported",
                key="kb_name",
            )
            kb_info = st.text_input(
                "Knowledge repository introduction",
                placeholder="Knowledge repository introduction, facilitating agent search",
                key="kb_info",
            )
        ...
        kb_info = st.text_area("Please enter a knowledge repository introduction", value=st.session_state["selected_kb_info"], max_chars=None,
                               key=None,
        ...
        with st.expander(
            ...
            chunk_overlap = cols[1].number_input("Overlap length of adjacent texts", 0, chunk_size, OVERLAP_SIZE)
            cols[2].write("")
            cols[2].write("")
            zh_title_enhance = cols[2].checkbox("Enable Chinese title enhancement", ZH_TITLE_ENHANCE)

        if st.button(
                "Add files to knowledge repository",
                # use_container_width=True,
        ...
        if not len(doc_details):
            ...
            st.write(f"The knowledge repository `{kb}`  already contains files")
            st.info("The knowledge repository contains both source files and vectorstore. Please select a file from the table below before proceeding.")
            ...
            gb = config_aggrid(
                doc_details,
                {
                    ...
                    ("in_db", "Vectorstore"): {"cellRenderer": cell_renderer},
                },
                "multiple",
            )
            ...
            # 将文件分词并加载到向量库中
            if cols[1].button(
                    "Re-add to vectorstore" if selected_rows and (
                            pd.DataFrame(selected_rows)["in_db"]).any() else "Add to vectorstore",
        ...
        with st.sidebar:
            keyword = st.text_input("Query keywords")
            ...
            if st.button("保存更改"):
                ...
                if changed_docs:
                    if api.update_kb_docs(knowledge_base_name=selected_kb,
                        ...
                        st.toast("Documents update successfully")
                    else:
                        st.toast("Documents update unsuccessfully")
