...
llm_llama_2_70b = QianfanChatEndpoint(streaming=True, temperature=0.5, top_p=0, penalty_score=1, model="Llama-2-70b-chat")
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    df2 = pd.DataFrame([dict(Question=Questions[i], Correct_Answer=Answers[i], Answer1=result_llm_llama_2_70b['result'],
                             Score1=final_score(Questions[i], result_llm_llama_2_70b['result'], Answers[i]),
                             Answer2=result_llm_ERNIE_Bot_turbo40['result'],
                             Score2=final_score(Questions[i], result_llm_ERNIE_Bot_turbo40['result'], Answers[i]),
                             Answer3=result_llm_ERNIE_Bot_turbo['result'],
                             Score3=final_score(Questions[i], result_llm_ERNIE_Bot_turbo['result'], Answers[i]))])

    print("No of Question",i+1,"\n","Right_Answer:",Answers[i],
          "\n"+"Answer_from_CLKR-empowered-llama_2_70b_with_local_knowledge:",str(result_llm_llama_2_70b['result']).strip().replace("\n",""),
          "\n"+"Answer_from_CLKR-empowered_ERNIE_Bot_turbo40:",str(result_llm_ERNIE_Bot_turbo40['result']).strip().replace("\n",""),
          "\n"+"Answer_from_CLKR-empowered_ERNIE_Bot_turbo:",str(result_llm_ERNIE_Bot_turbo['result']).strip().replace("\n",""))
    df = pd.concat([df, df2], axis=0)
  save_df_to_excel(df, "Answer_from_CLKR-empowered_llama-2-70b_ERNIE_Bot 4.0_ERNIE-Bot-turbo"+year+".xlsx", "sheet1")