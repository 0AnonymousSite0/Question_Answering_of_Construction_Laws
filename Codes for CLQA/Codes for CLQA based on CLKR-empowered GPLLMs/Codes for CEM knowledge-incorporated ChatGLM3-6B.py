# Modify LLM_MODEL = "chatglm3-6b" in model_config.py
#"chatglm3-6b": {
#    "name": "chatglm3-6b",
...
def get_file_names(folder):
    file_names = [str(folder) + r"/" + filename for filename in os.listdir(folder) if
...
def main():
    ...
    filepath = get_file_names(r"/root/autodl-tmp/langchain-ChatGLM/knowledge_base/CEMdocx/Allfiles_CEM")

    print(filepath)
    filepath = filepath.split(",")
    vs_path = r"/root/autodl-tmp/langchain-ChatGLM/knowledge_base/CEMdocx"
    ...
    for year in years:
        ...
        for i in range(len(Questions)):
            ...
            for j in answer1:
                answer = answer + j
            df2 = pd.DataFrame([{"Question": Questions[i], "Correct_Answer": Answers[i], "Answer1": answer1,
                                 "Score1": final_score(Questions[i], answer1, Answers[i])}])
            print("No Question", i + 1, "\n", "Right_Answer:", Answers[i],
                  "\nAnswer_from_CEM_knowledge-incorporated_ChatGLM3-6B:", str(answer).strip().replace("\n", ""))
            df = pd.concat([df, df2], axis=0)
        save_df_to_excel(df, "All_answers_from_CEM_knowledge-incorporated_ChatGLM3-6B" + year + ".xlsx", "sheet1")
