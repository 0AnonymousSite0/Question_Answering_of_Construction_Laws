...
documents = SimpleDirectoryReader(r'YOUR_KNOWLEDGE_BASE_PATH', recursive=True, exclude_hidden=True).load_data()
print("Local Knowledge are loaded")
db = chromadb.PersistentClient(path="./storage_of_CEM-EKB")
chroma_collection = db.get_or_create_collection("storage_of_CEM-EKB")
...
# Replace GPT-4 model/text-davinci-003
# llm = OpenAI(temperature=0, model="gpt-4/text-davinci-003")
# service_context = ServiceContext.from_defaults(llm=llm)
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No of Question",i+1,"\n","Right_Answer:",Answers[i],"\n"+"Answer_from_CEM_knowledge-incorporated_GPT-3.5-turbo:",str(answer1).strip().replace("\n",""))

    df = pd.concat([df, df2], axis=0)
  save_df_to_excel(df, "Answer_from_CLKR-empowered_GPT-3.5-turbo"+year+".xlsx", "sheet1")