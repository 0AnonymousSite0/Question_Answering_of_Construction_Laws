...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    with st.sidebar:
        ...
        def on_mode_change():
            ...
            text = f"已切换到 {mode} 模式。"
            if mode == "Knowledge repository question-answering":
        ...
        dialogue_modes = ["LLM chat",
                          "Knowledge repository chat",
                          "File chat",
                          "Search engine chat",
                          "Custom agent chat",
                          ]
        dialogue_mode = st.selectbox("Please select a dialogue mode",
                                     dialogue_modes,
                                     index=0,
        ...
        if cur_llm_model in llm_models:
            ...
            index = 0
        llm_model = st.selectbox("Please select a LLM model",
                                 llm_models,
                                 index,
        ...
        index_prompt = {
            "LLM chat": "llm_chat",
            "Custom agent chat": "agent_chat",
            "Search engine chat": "search_engine_chat",
            "Knowledge repository chat": "knowledge_base_chat",
        ...
        history_len = st.number_input("Historical dialogue rounds", 0, 20, HISTORY_LEN)

        def on_kb_change():
            st.toast(f"已加载知识库： {st.session_state.selected_kb}")

        if dialogue_mode == "Knowledge repository chat":
            with st.expander("knowledge repository configuration", True):
                kb_list = api.list_knowledge_bases()
                index = 0
                if DEFAULT_KNOWLEDGE_BASE in kb_list:
                    index = kb_list.index(DEFAULT_KNOWLEDGE_BASE)
                selected_kb = st.selectbox(
                    "Please select a knowledge repository",
                    kb_list,
                    index=index,
                    on_change=on_kb_change,
                    key="selected_kb",
                )
                kb_top_k = st.number_input("Number of matched knowledge items", 1, 20, VECTOR_SEARCH_TOP_K)

                ## Bge 模型会超过1
                score_threshold = st.slider("Knowledge match score threshold", 0.0, 2.0, float(SCORE_THRESHOLD), 0.01)
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            ...
            elif dialogue_mode == "Custom agent chat":
            ...
            elif dialogue_mode == "Knowledge repository chat":
                chat_box.ai_say([
